...
impl RadialBalloonTreeLayout {
    /// Predicts width and height of a node based on its contents
    fn predict_node_size(entry: &FsEntry, options: &LayoutOptions) -> (f32, f32) {
        ...
        let width = options.base_node_width * (0.85 + 0.45 * count_factor);
        let height = options.base_node_height * (0.85 + 0.45 * count_factor);
        (width, height)
    }
    ...
    /// Uses binary search to satisfy: sum( 2 * arcsin( (r_i + r_{i+1}) / (2 * R) ) ) <= 2*PI
    fn solve_minimal_orbit_radius(radii: &[f32], hub_radius: f32, gap: f32) -> f32 {
        ...
        let max_pairwise_dist = radii
            .iter()
            .enumerate()
            .map(|(i, &r)| r + radii[(i + 1) % n])
        ...
        for _ in 0..30 {
            ...
            if valid && angle_sum <= 2.0 * PI {
                high = mid; // R is large enough, try smaller
            } else {
                low = mid;  // R is too small, angles exceed 2*PI
            }
        }
    ...
    fn layout_radial_children(
        ...
        // 1. Calculate node dimensions and bounding radii for all children
        ...
        // 2. Solve for exact non-overlapping orbit radius
        let hub_radius = (options.base_node_width / 2.0).max(60.0);
        let orbit_radius = Self::solve_minimal_orbit_radius(&radii, hub_radius, options.gap);

        // 3. Compute exact non-overlapping angular positions
        let mut angles = Vec::with_capacity(n);
        let mut current_angle = -PI / 2.0; // Start at 12 o'clock

        // Compute exact angular step for each neighbor pair
        let mut raw_deltas = Vec::with_capacity(n);
        for i in 0..n {
            let d = radii[i] + radii[(i + 1) % n];
            let sin_val = (d / (2.0 * orbit_radius)).clamp(-1.0, 1.0);
...
impl CompoundLayoutAlgorithm for RadialBalloonTreeLayout {
    ...
    fn compute_layout(
        ...
        let padding = options.padding + 24.0;
